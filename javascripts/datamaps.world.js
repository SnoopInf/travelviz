      ...
      svg.selectAll('.datamaps-subunit')
        .on('mouseover', function(d) {
          ...
          if ( options.popupOnHover ) {
            self.updatePopup($this, d, options);
          }
        })
    ...
    bubbles
      .enter()
        ...
        .on('mouseover', function ( datum ) {
          var $this = d3.select(this);
            self.onMouseOver($this, datum);
            self.hexSelect(datum.id, datum.fillKey);
        })
      ...
      if ( self.options.zoomConfig.zoomOnClick ) {
          bubbles.on('click', function (d) { clickZoom.call(self, d) });
      }
    ...
    function clickZoom(d) {
        ...
        if ( d.radius ) { //Circle
            ...
            var cy = d3.select(d3.event.target).attr("cy");
                bounds = [ [ Number(cx) - d.radius, Number(cy) - d.radius ],[ Number(cx) + d.radius, Number(cy) + d.radius ] ];
            } else {
            ...
            self.svg.selectAll("path").classed("active", centered && function( d ) { return d === centered; });
            self.svg.selectAll("g").transition().duration(750).style("stroke-width", 1.5 / scale + "px").attr("transform", "translate(" + translate + ")scale(" + scale + ")");
        }
         ...
         function resetZoom() {
            ...
            this.svg.selectAll("g").transition().duration(750).style("stroke-width", "1.5px").attr("transform", "");
        }
...
;
  ...
  ***************************************/
  ...
  Datamap.prototype.updatePopup = function (element, d, options, external) {
    var self = this;
    //element.on('mousemove', null);
    //element.on('mousemove', function() {
    if(external){
        var position = [element.attr('cx'), element.attr('cy')];
    } else {
  ...
  Datamap.prototype.onMouseOver = function (element, datum, external) {
  var self = this;
  var options = this.options.bubblesOptions;
    if (options.highlightOnHover) {
              ...
              if (options.popupOnHover) {
                self.updatePopup(element, datum, options, external);
              }
  }

  Datamap.prototype.onMouseOut = function(element) {
     var options = this.options.bubblesOptions;
            if (options.highlightOnHover) {
              //reapply previous attributes
              var previousAttributes = JSON.parse( element.attr('data-previousAttributes') );
              for ( var attr in previousAttributes ) {
                element.style(attr, previousAttributes[attr]);
              }
            }
